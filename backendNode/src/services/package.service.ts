import type { Prisma, TravelPackage } from "@prisma/client";
...
function upcomingDateFilter(): Prisma.TravelPackageWhereInput {
  return {
    OR: [{ startDate: null }, { startDate: { gte: startOfToday() } }],
  };
}

function parseIndexedStringArray(
...
function whereForFilters(filters: PackageFilters): Prisma.TravelPackageWhereInput {
  const where: Prisma.TravelPackageWhereInput = {
    status: "ACTIVE",
    ...upcomingDateFilter(),
  };
  if (filters.transportation) where.transportation = filters.transportation;
...
function applyEffectivePriceFilter(
  rows: TravelPackage[],
...
async function includeResponse(pkg: TravelPackage) {
...
function sqDistance(aLat: number, aLng: number, bLat: number, bLng: number): number {
...
export async function listAll(filters: PackageFilters) {
  const where = Object.keys(filters).length
    ? whereForFilters(filters)
    : ({
        status: "ACTIVE",
        ...upcomingDateFilter(),
      } satisfies Prisma.TravelPackageWhereInput);
  ...
  return enriched.sort((a, b) => {
    const rank = (s: string) => (s === "ACTIVE" ? 1 : s === "FULL" ? 2 : 3);
...
export async function byType(type: string, filters: PackageFilters) {
...
export async function byTypeAndOrigin(type: string, originLat: number, originLong: number, filters: PackageFilters) {
  const where = whereForFilters(filters);
  where.packageType = type;
...
export async function byDestination(destinationLat: number, destinationLong: number) {
  const rows = await prisma.travelPackage.findMany({
...
export async function searchFromOrigin(
  origin: string,
  radiusKm = 20,
  ...
  const scored = rows
    .map((row) => {
      const distanceFromUserOrigin = haversineKm(
...
export async function searchNearby(
  ...
  const scored = rows
    ...
    .filter(
      (item) =>
        item.originDistanceKm <= radiusKm &&
        item.destinationDistanceKm <= radiusKm,
    )
    .sort(
      (a, b) =>
        a.originDistanceKm + a.destinationDistanceKm -
...
export async function myPackages(phone: string) {
...
async function uploadFilesToCloudinary(userKey: string, files: Express.Multer.File[]): Promise<string[]> {
  if (!files.length) return [];
  if (!hasCloudinaryConfig()) throw new ApiError(400, "Cloudinary is not configured");
...
async function notifySubscribers(pkg: TravelPackage, postedByName: string) {
...
export async function createPackage(phone: string, body: Record<string, unknown>, files: Express.Multer.File[]) {
  const user = await prisma.user.findUnique({ where: { phone } });
  if (!user) throw new ApiError(400, "User not found");
  ...
  const data: Prisma.TravelPackageCreateInput = {
...
export async function updatePackage(
  ...
  const pkg = await prisma.travelPackage.findUnique({ where: { id: BigInt(id) } });
  if (!pkg) throw new ApiError(400, "Package not found");
  if (pkg.userId !== user.id) throw new ApiError(400, "You can only update your own packages");
...
export async function deletePackage(id: number, phone: string) {
  const user = await prisma.user.findUnique({ where: { phone } });
  if (!user) throw new ApiError(400, "User not found");
  const pkg = await prisma.travelPackage.findUnique({ where: { id: BigInt(id) } });
  if (!pkg) throw new ApiError(400, "Package not found");
  if (pkg.userId !== user.id) throw new ApiError(400, "You can only delete your own packages");
...
export async function updateStatus(id: number, phone: string, status: string) {
  const user = await prisma.user.findUnique({ where: { phone } });
  if (!user) throw new ApiError(400, "User not found");
  const pkg = await prisma.travelPackage.findUnique({ where: { id: BigInt(id) } });
  if (!pkg) throw new ApiError(400, "Package not found");
