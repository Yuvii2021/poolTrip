...
packageRouter.get(
  "/type/:type/originLat/:originLat/originLong/:originLong",
  asyncHandler(async (req, res) => {
    const filters = packageQuerySchema.parse(req.query);
    const rows = await packageService.byTypeAndOrigin(
      req.params.type,
      Number(req.params.originLat),
      Number(req.params.originLong),
      filters,
    );
    res.json(rows);
  }),
...
packageRouter.get(
  "/destination/destinationLat/:destinationLat/destinationLong/:destinationLong",
  asyncHandler(async (req, res) => {
...
packageRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const filtered = rows.filter((row) =>
      [row.title, row.destination, row.origin, row.description]
        .filter(Boolean)
...
packageRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const selectedOrigin =
      Number.isFinite(originLat) && Number.isFinite(originLong)
        ? { latitude: originLat, longitude: originLong }
        : null;
    const selectedDestination =
      Number.isFinite(destinationLat) && Number.isFinite(destinationLong)
    ...
    res.json(
      await packageService.searchNearby(
        origin,
        destination,
        Number.isNaN(radiusKm) ? 20 : radiusKm,
        selectedOrigin,
        selectedDestination,
      ),
    );
...
packageRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(
      await packageService.searchFromOrigin(
        origin,
        Number.isNaN(radiusKm) ? 20 : radiusKm,
        selectedOrigin,
      ),
