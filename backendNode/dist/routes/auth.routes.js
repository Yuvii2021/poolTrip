...
import { authRequestSchema, forgotPasswordSchema, registerSchema, resetPasswordSchema, sendEmailOtpSchema, updateMeSchema, verifyEmailOtpSchema, } from "../domain/validators.js";
...
authRouter.get("/user/:id", asyncHandler(async (req, res) => {
...
authRouter.post("/email/send-otp", requireAuth, asyncHandler(async (req, res) => {
    const payload = sendEmailOtpSchema.parse(req.body);
    await authService.sendEmailOtp(req.userPhone, payload.email);
    res.json({ message: "OTP has been sent to your email" });
}));
authRouter.post("/email/verify-otp", requireAuth, asyncHandler(async (req, res) => {
    const payload = verifyEmailOtpSchema.parse(req.body);
    await authService.verifyEmailOtp(req.userPhone, payload.otp);
    res.json({ message: "Email verified successfully" });
}));
authRouter.post("/me/profile-photo", requireAuth, upload.single("photo"), asyncHandler(async (req, res) => {
