...
bookingRouter.post("/:id/approve", asyncHandler(async (req, res) => {
    const response = await bookingService.approveBooking(req.userPhone, Number(req.params.id));
    res.json(response);
}));
bookingRouter.post("/:id/reject", asyncHandler(async (req, res) => {
...
bookingRouter.get("/my", asyncHandler(async (req, res) => {
    res.json(await bookingService.myBookings(req.userPhone));
}));
bookingRouter.get("/host", asyncHandler(async (req, res) => {
    res.json(await bookingService.hostBookings(req.userPhone));
}));
bookingRouter.get("/host/pending", asyncHandler(async (req, res) => {
    res.json(await bookingService.hostBookings(req.userPhone, true));
}));
bookingRouter.get("/status/:packageId", asyncHandler(async (req, res) => {
    res.json(await bookingService.bookingStatus(req.userPhone, Number(req.params.packageId)));
}));
bookingRouter.post("/:id/rate", asyncHandler(async (req, res) => {
