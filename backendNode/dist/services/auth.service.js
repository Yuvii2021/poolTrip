...
export function signToken(email, phone) {
...
export async function getCurrentUser(phone) {
    const user = await prisma.user.findUnique({ where: { phone } });
    if (!user)
...
export async function updateCurrentUser(phone, updates) {
    const user = await prisma.user.findUnique({ where: { phone } });
    if (!user)
        throw new ApiError(400, "User not found");
    const patch = {};
    ...
    if (updates.whatsappNumber?.trim())
        patch.whatsappNumber = updates.whatsappNumber.trim();
    if (updates.bio !== undefined)
        patch.bio = updates.bio.trim() || null;
...
export async function sendOtpForRegistration(phone) {
...
export async function forgotPassword(phone) {
    const user = await prisma.user.findUnique({ where: { phone } });
    if (!user)
        throw new ApiError(400, "User not found with this phone number");
...
function otpKey(fullName, email) {
...
export async function uploadProfilePhoto(phone, file) {
    const user = await prisma.user.findUnique({ where: { phone } });
    if (!user)
        throw new ApiError(400, "User not found");
    if (!file)
