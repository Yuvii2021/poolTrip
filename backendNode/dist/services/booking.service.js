...
async function hydrateBooking(bookingId) {
...
export async function createBooking(phone, input) {
    ...
    if (existing)
        throw new ApiError(400, "You already have an active booking for this trip");
...
export async function approveBooking(phone, bookingId) {
    ...
    if (!pkg)
        throw new ApiError(400, "Package not found");
    if (pkg.userId !== host.id)
        throw new ApiError(400, "Only the trip host can approve bookings");
    if (booking.status !== BookingStatus.PENDING)
        throw new ApiError(400, "This booking is not pending");
    if (pkg.availableSeats < booking.seatsBooked)
        throw new ApiError(400, "Not enough seats available anymore");
...
export async function rejectBooking(phone, bookingId) {
    const host = await prisma.user.findUnique({ where: { phone } });
    if (!host)
        throw new ApiError(400, "User not found");
    const booking = await prisma.booking.findUnique({ where: { id: BigInt(bookingId) } });
    ...
    if (!pkg)
        throw new ApiError(400, "Package not found");
    if (pkg.userId !== host.id)
        throw new ApiError(400, "Only the trip host can reject bookings");
    if (booking.status !== BookingStatus.PENDING)
        throw new ApiError(400, "This booking is not pending");
    await prisma.booking.update({
        where: { id: booking.id },
...
export async function cancelBooking(phone, bookingId) {
    const passenger = await prisma.user.findUnique({ where: { phone } });
    if (!passenger)
        throw new ApiError(400, "User not found");
    const booking = await prisma.booking.findUnique({ where: { id: BigInt(bookingId) } });
    ...
    if (booking.passengerId !== passenger.id)
        throw new ApiError(400, "You can only cancel your own bookings");
...
export async function myBookings(phone) {
...
export async function hostBookings(phone, pendingOnly = false) {
    const host = await prisma.user.findUnique({ where: { phone } });
    if (!host)
        throw new ApiError(400, "User not found");
    const hostPackageIds = (await prisma.travelPackage.findMany({
        where: { userId: host.id },
        select: { id: true },
    })).map((x) => x.id);
    if (!hostPackageIds.length)
        return [];
    const rows = await prisma.booking.findMany({
        where: {
...
export async function bookingStatus(phone, packageId) {
    const user = await prisma.user.findUnique({ where: { phone } });
    if (!user)
        throw new ApiError(400, "User not found");
    const existing = await prisma.booking.findFirst({
...
export async function rateBooking(phone, bookingId, rating, review) {
    const passenger = await prisma.user.findUnique({ where: { phone } });
    if (!passenger)
        throw new ApiError(400, "User not found");
    const booking = await prisma.booking.findUnique({ where: { id: BigInt(bookingId) } });
    if (!booking)
        throw new ApiError(400, "Booking not found");
    if (booking.passengerId !== passenger.id)
        throw new ApiError(400, "You can only rate your own bookings");
    if (booking.status !== BookingStatus.CONFIRMED)
        throw new ApiError(400, "Only confirmed bookings can be rated");
    if (booking.rating !== null)
        throw new ApiError(400, "You have already rated this booking");
    const pkg = await prisma.travelPackage.findUnique({ where: { id: booking.packageId } });
